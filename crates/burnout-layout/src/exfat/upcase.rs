//! The up-case table of exFAT.
//!
//! exFAT compares names without case, and the volume itself says how: a table
//! gives the upper case of each UTF-16 unit. A host reads the table from the
//! volume, so the table that gives the hash of each name has to be the table
//! that goes onto the volume.
//!
//! The table here is the one that the specification recommends, in the
//! compressed form of its Table 25. A run of units that are their own upper
//! case is FFFFh and the length of the run. Each line of the array holds the
//! eight entries of one line of Table 25, so a reader can hold the two side
//! by side.

/// The checksum of [`RECOMMENDED`] on a volume, as the specification gives it.
pub(crate) const RECOMMENDED_CHECKSUM: u32 = 0xE619_D30D;

/// The number of UTF-16 units, each of which the table maps.
const UNITS: usize = 0x1_0000;

/// The unit that starts a run of units that are their own upper case.
const RUN: u16 = 0xFFFF;

/// The bytes of the recommended table, as they go onto a volume.
pub(crate) fn recommended_bytes() -> Vec<u8> {
    RECOMMENDED
        .iter()
...
/// The upper case of each of the 65,536 UTF-16 units.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct UpCase(Vec<u16>);

impl UpCase {
    /// The table that the specification recommends.
    pub(crate) fn recommended() -> Self {
        Self::from_table(&RECOMMENDED).expect("the recommended table is a valid table")
    }

    /// A table as a volume holds it, compressed or not.
    ///
    /// `None` when it is not a table: a run with no length, entries past the
    /// last unit, or a first 128 units that do not map as the specification
    /// requires.
    pub(crate) fn from_table(table: &[u16]) -> Option<Self> {
        let map = expand(table)?;
        let mandatory = (0..128u16).all(|unit| map[unit as usize] == ascii_upper(unit));
        mandatory.then_some(UpCase(map))
    }

    /// A table from its bytes on a volume.
    ...
    pub(crate) fn unit(&self, unit: u16) -> u16 {
        self.0[unit as usize]
    }

    /// A name in upper case, one unit at a time.
    pub(crate) fn name(&self, name: &[u16]) -> Vec<u16> {
        name.iter().map(|unit| self.unit(*unit)).collect()
    }
}

/// The upper case of an ASCII unit, which is the mandatory part of a table.
...
/// The full map from a table in either form.
///
/// An entry that equals its own unit maps that unit to itself, even when it
/// is FFFFh. That is how the last entry of the recommended table reads, and
/// it is how Linux reads a table. Any other FFFFh starts a run, and the entry
/// after it is the length of the run. The units after the last entry map to
/// themselves.
fn expand(table: &[u16]) -> Option<Vec<u16>> {
    let mut map: Vec<u16> = Vec::with_capacity(UNITS);
    let mut entries = table.iter().copied();
    while let Some(entry) = entries.next() {
        let unit = map.len();
        if unit == UNITS {
            return None;
        }
        if entry as usize == unit || entry != RUN {
            map.push(entry);
            continue;
        }
        let run = entries.next()? as usize;
        if unit + run > UNITS {
            return None;
        }
        map.extend((unit..unit + run).map(|u| u as u16));
    }
    let tail = map.len()..UNITS;
    map.extend(tail.map(|u| u as u16));
    Some(map)
}

/// The recommended up-case table, compressed, as Table 25 of the exFAT
/// specification gives it.
#[rustfmt::skip]
pub(crate) const RECOMMENDED: [u16; 2918] = [
...
mod tests {
    use super::*;
    use crate::exfat::sums::table_checksum;

    #[test]
    fn the_recommended_table_has_the_checksum_that_the_specification_gives() {
        let bytes = recommended_bytes();
        assert_eq!(bytes.len(), 5836);
        assert_eq!(table_checksum(&bytes), RECOMMENDED_CHECKSUM);
    }

    #[test]
    fn the_first_128_units_map_as_the_specification_requires() {
        let up = UpCase::recommended();
        for unit in 0..128u16 {
            let want = if (0x61..=0x7A).contains(&unit) {
                unit - 0x20
            } else {
                unit
            };
            assert_eq!(up.unit(unit), want, "{unit:#06x}");
        }
    }

    #[test]
    fn letters_past_ascii_have_their_upper_case() {
        let up = UpCase::recommended();
        for (lower, upper) in [
            ('\u{fc}', '\u{dc}'),
            ('\u{ff}', '\u{178}'),
            ('\u{3b1}', '\u{391}'),
            ('\u{430}', '\u{410}'),
            // After the first run of the table, which is 6,134 units long.
            ('\u{1d7d}', '\u{2c63}'),
            // After the last run.
            ('\u{ff41}', '\u{ff21}'),
        ] {
            assert_eq!(up.unit(lower as u16), upper as u16, "{lower}");
        }
    }

    #[test]
    fn a_unit_with_no_single_upper_case_stays_as_it_is() {
        // The upper case of the German sharp s is two letters, which one unit
        // cannot hold.
        let up = UpCase::recommended();
        assert_eq!(up.unit(0xDF), 0xDF);
        assert_eq!(up.unit(0x4E2D), 0x4E2D);
    }

    #[test]
    fn the_last_entry_maps_ffff_to_itself_and_starts_no_run() {
        assert_eq!(UpCase::recommended().unit(0xFFFF), 0xFFFF);
    }

    #[test]
    fn a_run_stands_for_units_that_are_their_own_upper_case() {
        // The example of the specification: 97 units that map to themselves,
        // and then a, b and c.
        let map = expand(&[0xFFFF, 0x0061, 0x0041, 0x0042, 0x0043]).unwrap();
        assert_eq!(map.len(), UNITS);
        assert_eq!(map[0x60], 0x60);
        assert_eq!(&map[0x61..0x64], &[0x41, 0x42, 0x43]);
        assert_eq!(map[0x64], 0x64);
    }

    #[test]
    fn a_table_that_is_not_compressed_gives_the_same_map() {
        let whole = UpCase::recommended();
        let table: Vec<u16> = (0..=0xFFFFu16).map(|unit| whole.unit(unit)).collect();
        assert_eq!(UpCase::from_table(&table), Some(whole));
    }

    #[test]
    fn a_broken_table_is_not_a_table() {
        // A run with no length after it.
        assert_eq!(expand(&[0x0000, 0xFFFF]), None);
        // A run past the last unit.
        assert_eq!(expand(&[0xFFFF, 0xFFFF, 0x0041, 0x0042]), None);
        // More entries than units.
        assert_eq!(expand(&vec![0x0041; UNITS + 1]), None);
        // Bytes that are not whole units.
        assert_eq!(UpCase::from_bytes(&[0x00, 0x00, 0x01]), None);
    }

    #[test]
    fn a_table_that_breaks_the_mandatory_units_is_refused() {
        // It maps a to itself, so it does not compare names as exFAT does.
        let mut table: Vec<u16> = RECOMMENDED.to_vec();
        table[0x61] = 0x61;
        assert_eq!(UpCase::from_table(&table), None);
    }

    #[test]
    fn a_name_goes_to_upper_case_one_unit_at_a_time() {
        let up = UpCase::recommended();
        let name: Vec<u16> = "\u{dc}berpr\u{fc}fung.txt".encode_utf16().collect();
        let want: Vec<u16> = "\u{dc}BERPR\u{dc}FUNG.TXT".encode_utf16().collect();
        assert_eq!(up.name(&name), want);
    }
}
